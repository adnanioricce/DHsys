...
namespace Services.Billing
{
    public class BillingService : IBillingService
    {
        ...
        {
            return _billingRepository.Query().Where(b => !b.IsPaid).Take(limit is null ? 100 : limit).ToList();
        }
        public async Task<IEnumerable<Billing>> GetAllBillingsByBeneficiaryId(int beneficiaryId)
        {
