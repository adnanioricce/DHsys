...
{    
    ...
    {
        public static readonly string MigrationProjectDir = Directory.GetDirectoryRoot(Assembly.GetAssembly(typeof(DAL.DALAssembly)).Location);
        /// <summary>
        ...
        public static void AddMigration(string migrationName, ContextType contextType)
        {
            if(contextType == ContextType.Local)
            {
                ...
                var migrationFiles = localContext.AddMigration(MigrationProjectDir,"Migrations/Local",string.Format("{0}-local",migrationName));
            ...
            else if(contextType == ContextType.Remote)
            {
                ...
                var migrationFiles = remoteContext.AddMigration(MigrationProjectDir, "Migrations/Local", string.Format("{0}-local", migrationName));
                Console.WriteLine("--- Files created ---");
                Console.WriteLine("MetadataFile: {0}", migrationFiles.MetadataFile);
                Console.WriteLine("MigrationFile: {0}", migrationFiles.MigrationFile);
                Console.WriteLine("SnapshotFile: {0}", migrationFiles.SnapshotFile);
